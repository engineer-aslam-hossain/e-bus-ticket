...
import seatStatus from '../../fakeData/seatStatus.json';
...
const SearchResult: React.FC<Props> = (props) => {
  ...
  const bookingListOfTheBus: any = bookings.filter(
    (booking: any) => booking.busId === id
  );
  return (
    <div className='bg-white p-4 mb-2'>
      <div className='row align-items-center specs'>
        <div className='col-md-3 text-left'>
          ...
          <h6>{type}</h6>
          <p>
            Staring Point : <span className='text-danger'>{startingPoint}</span>
          </p>
          <p>
            End Point : <span className='text-danger'>{endPoint}</span>
          </p>
        </div>
        ...
        <div className='col-md-2'>
          ...
          <h6 className='text-danger'>
            {seats.length - bookingListOfTheBus.length}
          </h6>
        </div>
        <div className='col-md-3'>
          <div className='d-flex justify-content-between'>
            ...
            <button
              ...
              className={`btn btn-${showSeatDetails ? 'info' : 'danger'}`}
            >
              {showSeatDetails ? 'Hide details' : 'View Seats'}
            </button>
          </div>
        </div>
      </div>
